...
// Query, domain and scope the fixture proofs were generated for
const facematchQuery = { facematch: { mode: "regular", passed: true } }
const fixtureBinding = { domain: "localhost", scope: "age-check" }
...
describe("POST /verify", () => {
  ...
  it("should return verified: true for valid proofs with a matching query", async () => {
    const res = await app.inject({
      ...
      payload: {
        ...
        serviceConfig: {
          ...fixtureBinding,
          validityPeriodInSeconds: TEN_YEARS_IN_SECONDS,
          devMode: true,
        },
      },
    })
  ...
  it("should return verified: false for an aged proof without ignoreValidity", async () => {
    const res = await app.inject({
      ...
      payload: {
        ...
        originalQuery: facematchQuery,
        queryResult: facematchQuery,
        serviceConfig: { ...fixtureBinding, devMode: true },
      },
    })
  ...
  it("should return verified: false when the domain and scope are omitted", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/verify",
      payload: {
    ...
    assert.equal(res.statusCode, 400)
    assert.equal(res.json().verified, false)
    assert.match(res.json().error, /scope/)
  })
...
const bundleDate = (bundle: { name: string; proof: string }[]) => {
  const outer = bundle.find((p) => p.name.startsWith("outer"))
  if (outer) {
    return getCurrentDateFromOuterProof(
      getProofData(outer.proof, getNumberOfPublicInputs(outer.name)),
    )
  }
  const disclosure = bundle.find(
    (p) =>
...
describe("POST /verify — proof modes", () => {
  ...
  for (const [mode, file, expectedId, expectedIdType] of [
    ...
    it(`should verify a ${mode} bundle`, async () => {
      const request = loadModeFixture(file)
      mock.timers.enable({ apis: ["Date"], now: bundleDate(request.proofs) })
      try {
  ...
  // Verified by a call to the verifier contract, which uses its own clock, so this relies on
  // ignoreValidity instead of a pinned Date to keep passing as the fixture ages
  it("should verify a compressed-evm bundle against the on-chain verifier", async () => {
    const request = loadModeFixture("verify-compressed-evm-request.json")
    const res = await app.inject({
      method: "POST",
      url: "/verify",
      payload: { ...request, options: { ignoreValidity: true } },
    })
    const json = res.json()
    assert.equal(json.verified, true, `Expected verified: true, got error: ${json.error}`)
    assert.equal(res.statusCode, 200)
